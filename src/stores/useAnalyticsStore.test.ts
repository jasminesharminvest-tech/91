...
describe('useAnalyticsStore', () => {
  ...
  describe('maybeRunAutoCleanup', () => {
    // Реальные проблемы, п.6: раньше история могла очищаться только
    // вручную. Теперь: первый вызов только фиксирует точку отсчёта, не
    // удаляя ничего, а последующий вызов спустя >= 24ч действительно чистит.
    it('does not clear anything on the very first call, only records the timestamp', () => {
      useAnalyticsStore.getState().addSignal(makeSignal({ id: 's1', outcome: 'win' }));
    ...
    it('does not clear again before 24 hours have passed since the last cleanup', () => {
      useAnalyticsStore.getState().addSignal(makeSignal({ id: 's1', outcome: 'win' }));
      useAnalyticsStore.setState({ lastAutoCleanupAt: Date.now() - 60 * 60 * 1000 }); // 1h ago
      const ran = useAnalyticsStore.getState().maybeRunAutoCleanup();
      expect(ran).toBe(false);
      expect(useAnalyticsStore.getState().signals).toHaveLength(1);
    });

    it('clears signals, currentSignal, winRate and calibrationSampleCount once 24h have passed', () => {
      useAnalyticsStore.getState().addSignal(makeSignal({ id: 's1', outcome: 'win' }));
      useAnalyticsStore.getState().setCurrentSignal(makeSignal({ id: 'current' }));
      useAnalyticsStore.getState().recomputeStats();
      useAnalyticsStore.setState({ lastAutoCleanupAt: Date.now() - 25 * 60 * 60 * 1000 }); // 25h ago
